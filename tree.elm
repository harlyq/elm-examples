...
main =
    flow down
        ...
        , display "isElement" (isElement 5) t1
        , display "fold" (fold (+) 10) t1
        , display "sum2" (fold (+) 0) t2
        , display "flatten2" (fold (::) []) t1
        , display "isElement" (fold (\x y -> y || x == 5) False) t1
        ]
...
isElement value tree =
    case tree of
      Empty -> False
      Node v left right ->
        if v == value then True else (isElement value left) || (isElement value right)
...
fold func x tree =
    case tree of
      Empty -> x
      Node v left right ->
        fold func (fold func (func v x) left) right
